...
export default function FloatingInput({ label, icon: Icon, name, value, onChange }) {
    return (
        ...
        <div className="flex items-center border border-gray-300 rounded-md px-2 py-1 bg-gray-50 
            ...
            className="peer border-0 shadow-none bg-transparent text-sm text-gray-700 
                        placeholder-transparent focus-visible:ring-0 h-8"
            />
            <label
            htmlFor={name}
            className={`absolute transition-all pointer-events-none
                ${value
                ? "-top-2 left-8 text-xs text-blue-600"
                ...
                peer-focus:-top-2 peer-focus:left-8 peer-focus:translate-y-0 peer-focus:text-xs peer-focus:text-blue-600
                bg-gray-50 px-1`}
            >
            {label}
            </label>
